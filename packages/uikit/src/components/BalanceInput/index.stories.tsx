...
export const UnitDisplay: React.FC<React.PropsWithChildren> = () => {
  ...
  const cakeToUSD = (input: string) => {
    const convertedToUSD = parseFloat(input) * MIND_PRICE;
...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const MIND_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"MIND" | "USD">("MIND");
  const conversionUnit = editingUnit === "MIND" ? "USD" : "MIND";
  const [values, setValues] = useState({
    MIND: "1006.086957",
    USD: `${1006.086957 * MIND_PRICE}`,
  });
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "MIND" ? "USD" : "MIND";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "MIND") {
      setValues({
        MIND: input,
        USD: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat * MIND_PRICE}`,
      });
    } else {
      setValues({
        MIND: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / MIND_PRICE}`,
        USD: input,
      });
    }
  };
