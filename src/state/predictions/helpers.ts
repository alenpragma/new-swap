...
import { GRAPH_API_PREDICTION_BNB, GRAPH_API_PREDICTION_MIND } from 'config/constants/endpoints'
...
import { transformBetResponseMIND, transformUserResponseMIND } from './cakeTransformers'
...
import { BetResponseMIND } from './cakeQueries'
...
export const transformBetResponse = (tokenSymbol) =>
  tokenSymbol === 'MIND' ? transformBetResponseMIND : transformBetResponseBNB

export const transformUserResponse = (tokenSymbol) =>
  tokenSymbol === 'MIND' ? transformUserResponseMIND : transformUserResponseBNB
...
export const getTotalWon = async (): Promise<{ totalWonBNB: number; totalWonMIND: number }> => {
  const [{ market: BNBMarket, market: MINDMarket }] = await Promise.all([
    request(
  ...
  const totalWonMIND = getTotalWonMarket(MINDMarket, 'MIND')

  return { totalWonBNB, totalWonMIND }
}
...
export const getBetHistory = async (
  ...
  api: string,
  tokenSymbol: string,
): Promise<Array<BetResponseBNB | BetResponseMIND>> => {
