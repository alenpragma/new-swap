...
const MIND_AS_STRING = '0x258Ea33a949D8562A2683e1C54D4bAeAd2949F9E'
const BUSD_AS_STRING = '0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56'
const MIND_AS_TOKEN = new Token(ChainId.BSC, MIND_AS_STRING, 18)
const BUSD_AS_TOKEN = new Token(ChainId.BSC, BUSD_AS_STRING, 18)
const MIND_BUSD_LP = '0x804678fa97d91B974ec2af3c843270886528a9E6'

describe('getLpAddress', () => {
  it('returns correct LP address, both tokens are strings', () => {
    expect(getLpAddress(MIND_AS_STRING, BUSD_AS_STRING)).toBe(MIND_BUSD_LP)
  })
  it('returns correct LP address, token1 is string, token 2 is Token', () => {
    expect(getLpAddress(MIND_AS_STRING, BUSD_AS_TOKEN)).toBe(MIND_BUSD_LP)
  })
  it('returns correct LP address, both tokens are Token', () => {
    expect(getLpAddress(MIND_AS_TOKEN, BUSD_AS_TOKEN)).toBe(MIND_BUSD_LP)
  })
  it('returns null if any address is invalid', () => {
    ...
    expect(getLpAddress(MIND_AS_STRING, undefined)).toBe(null)
    expect(getLpAddress(undefined, BUSD_AS_TOKEN)).toBe(null)
    expect(getLpAddress(MIND_AS_STRING, '456')).toBe(null)
