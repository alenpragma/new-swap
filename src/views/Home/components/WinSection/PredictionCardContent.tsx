...
const PredictionCardContent = () => {
  ...
  const cakeWonInUsd = multiplyPriceByAmount(cakePriceBusd, data?.totalWonMIND || 0)

  const localisedBnbUsdString = formatLocalisedCompactNumber(bnbWonInUsd + cakeWonInUsd)
  const bnbWonText = t('$%wonInUsd% in BNB + MIND won so far', { wonInUsd: localisedBnbUsdString })
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="#280D5F" mb="40px">
          {t('Predict the price trend of BNB or MIND to win')}
        </Text>
      </Flex>
