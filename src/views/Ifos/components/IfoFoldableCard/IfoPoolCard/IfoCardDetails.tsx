...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of MIND up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max MIND entry is capped by minimum between your average MIND balance in the iMIND, or the pool’s hard cap. To increase the max entry, Stake more MIND into the iMIND',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, Max MIND entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iMIND.')}{' '}
        </Text>
        <Text display="inline">
          {t('Lock more MIND for longer durations to increase the maximum number of MIND you can commit to the sale.')}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, Max MIND entry is capped by your average MIND balance in the iCake. To increase the max entry, Stake more MIND into the iCake',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. MIND entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('MIND to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
